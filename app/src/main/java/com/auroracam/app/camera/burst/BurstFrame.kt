...
data class RawYuvFrame(
    ...
    companion object {
        ...
        fun fromImage(
            ...
            var yDirect: ByteBuffer? = null
            var uDirect: ByteBuffer? = null
            var vDirect: ByteBuffer? = null

            try {
                // 1. Extract Y plane into contiguous direct buffer (width x height), drawn from pool
                yDirect = YuvBufferPool.acquire(width * height)
                ...
                uDirect = YuvBufferPool.acquire(uvWidth * uvHeight)
                vDirect = YuvBufferPool.acquire(uvWidth * uvHeight)
                ...
                yDirect?.let { YuvBufferPool.release(it) }
                uDirect?.let { YuvBufferPool.release(it) }
                vDirect?.let { YuvBufferPool.release(it) }
                throw t
            }
        }
    }
